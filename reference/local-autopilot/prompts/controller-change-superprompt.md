...
You are operating with the persistent `BBDevOpsAutopilot` setup. Execute the full `psc-sre-automacao-controller` release flow end to end with no manual steps after this prompt.

## Mandatory behavior
- Discover all runtime paths, repo URLs, web links, branch rules, and deploy targets from `<LOCAL_USER_HOME>\.bbdevops-autopilot-safe\BBDevOpsAutopilot\autopilot-manifest.json` before making changes.
- Treat controller work as a managed pair of repositories:
  - source/controller repo: `https://github.com/<OWNER>/psc-sre-automacao-controller`
  - deploy repo for homologa̤̣o tag promotion: `https://github.com/<OWNER>/deploy-psc-sre-automacao-controller`
...
- Work only in the canonical controller clone at `<LOCAL_USER_HOME>\.bbdevops-autopilot-safe\BBDevOpsAutopilot\repos\psc-sre-automacao-controller`.
...
- After the controller CI finishes with success, update `deploy-psc-sre-automacao-controller` on branch `cloud/homologacao`, setting `deployment.containers.tag` in `values.yaml` to the same release version.
- Stop the default release flow after deploy promotion, when `deployment.containers.tag` in `cloud/homologacao` has been updated to the new release version.
...
- Do not depend on `<LOCAL_USER_HOME>\<CORPORATE_ONEDRIVE>\AUTOMACAO`; it may be missing.
