$spoolDir = "<LOCAL_USER_HOME>\.bbdevops-autopilot-safe\BBDevOpsAutopilot\spooler"
$autosyncScript = "<LOCAL_USER_HOME>\.bbdevops-autopilot-safe\BBDevOpsAutopilot\agent-autosync.ps1"
...
while ($true) {
    ...
    if (Test-Path $pending) {
        ...
        try {
            $output = & "powershell.exe" -ExecutionPolicy Bypass -File $pending 2>&1
